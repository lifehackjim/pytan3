...
def exception_hook(type, value, tb):
    ...
    Notes:
        If the package logger has no logging output at "error" level,
        the default excepthook will be used to print the exception to STDERR.

    """
    if issubclass(type, (KeyboardInterrupt, SystemExit)):  # pragma: no cover
        sys.__excepthook__(type, value, tb)
        return

    if not will_print_at(obj=LOG, lvl="error"):  # pragma: no cover
        nvalue = "Uncaught Exception and no log output at 'ERROR' level for {log}"
        nvalue = nvalue.format(log=log_str(LOG))
        print(nvalue, file=sys.stderr)
        sys.__excepthook__(type, value, tb)
        return

    error = "Uncaught Exception!"  # pragma: no cover
    LOG.error(error, exc_info=(type, value, tb))  # pragma: no cover
...
def warning_hook(message, category, filename, lineno, file=None, line=None):
    ...
    Notes:
        If the package logger has no logging output at "warning" level,
        the default warning will be used to print the warning to STDERR.

    """
    if not will_print_at(obj=LOG, lvl="warning"):  # pragma: no cover
    ...
    m = warnings.formatwarning(  # pragma: no cover
        message=message, category=category, filename=filename, lineno=lineno, line=line
    )
    LOG.warning(m)  # pragma: no cover


def customformatwarning(message, category, filename, lineno, file=None, line=None):
    """Return a custom warning format."""
    m = "!!! {c} {fn}:{ln}:\n{m}\n"
    m = m.format(fn=filename, ln=lineno, c=category.__name__, m=message)
    return m
...
enable_capture_warn()
# Capture warnings to the logging system by default
enable_capture_exc()
# Capture exceptions to the logging system by default
