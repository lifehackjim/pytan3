...
def api_fixes():
    ...
    models["ParseQuestionResult"].API_COMPLEX = fix
...
class ApiList(ApiModel, api_models.ApiList):
    ...
    def api_coerce_items_hook(self, attr, value, op):
        ...
        Notes:
            REST arrays can include a CacheInfo object in them. This checks to see if
            any item is a CacheInfo object, and if so:
        ...
        """
        cache_info_keys = ["cache_id", "cache_row_count"]
        models = get_api_all_model()

        items = []
        for item in value:
            if isinstance(item, dict) and all(k in item for k in cache_info_keys):
                self.API_COMPLEX["cache_info"] = models["CacheInfo"]
                self.cache_info = item
                continue
            items.append(item)
        return items
...
class ResultSet(ApiItem):
    ...
    API_COMPLEX = {"columns": "ColumnList", "rows": "RowList"}
    """:obj:`dict`: Map of complex attributes to their types."""
    ...
    def __init__(self, **kwargs):
        ...
        self.columns = ColumnList() if self.columns is None else self.columns
        self.rows = RowList() if self.rows is None else self.rows
    ...
    def __getitem__(self, value):
        ...
        Args:
            value (:obj:`int` or :obj:`str`):
                If int: Index of item to retrieve from self.
...
class Row(ApiItem):
    ...
    API_COMPLEX = {"data": "RowColumnList"}
    """:obj:`dict`: Map of complex attributes to their types."""
    ...
    def __getitem__(self, value):
        """Support indexing of :attr:`columns`.

        Args:
            value (:obj:`int` or :obj:`str`):
                If int: Index of item to retrieve from :attr:`columns`.
                If str: Column name to index correlate from :attr:`ColumnList.names`

        Returns:
            :attr:`RowColumn`:
                The RowColumn object at index value from :attr:`columns`.
...
class RowValue(ApiItem):
    ...
    API_SIMPLE = {"hash": "integer_types", "text": "string_types"}
    """:obj:`dict`: Map of simple attributes to their types."""
...
class ParseQuestionResultList(ApiList):
    """Manually defined API array object."""
    ...
    API_ITEM_CLS = "ParseQuestionResult"
    """:class:`ApiItem`: Item class this list class holds."""


class ParseQuestionResult(ApiItem):
    """Manually defined API object."""

    API_NAME = "parse_question_result"
    """:obj:`str`: Name of object used in API calls."""
    # used to be 'question', but name conflict with 'Question'

    API_NAME_SRC = ""
    """:obj:`str`: Name of object in source file."""

    API_SIMPLE = {
        "from_canonical_text": "integer_types",
        "question_text": "string_types",
        "force_computer_id_flag": "integer_types",
    }
    """:obj:`dict`: Map of simple attributes to their types."""
    ...
    API_LIST_CLS = "ParseQuestionResultList"
    """:class:`ApiList`: List class that holds this item class."""
...
class LdapSyncConnector(ApiItem):
    ...
    API_SIMPLE = {
        ...
        "sync_individual_members_flag": "integer_types",  # noqa: E501
    }
    """:obj:`dict`: Map of simple attributes to their types."""
...
BUILD_META = {
    ...
    "script_platform": "Darwin-18.2.0-x86_64-i386-64bit",
    # OS Platform that ran this build script
    "script_python": "3.7.2 (default, Feb 20 2019, 14:09:01) ",
    # Python version that ran this build script
    "date": "2019-02-22 07:46:08.408656",
    ...
    "source_file_date": "2019-02-22 07:29:24.912505",
